...
#      #rintinvoice.date[0:4], invoice.date, current.year
...
#      if str(employee.date[0:4])==str(current.year):
    ...
    extra_save=save_prefix+month
    for invoice in unfiltered_invoice_array:
#      #rintstr(int(invoice.date[5:7])), str(current.month), invoice.date
...
#  #rintemployee_time_cards
  ...
  for employee in employee_time_cards:
#    #rintemployee.employee_name, "finitio"
...
#        monthly_invoice_array.append(invoice)
  #rintmonthly_invoice_array
  #rintmonthly_time_cards_array 
  ...
  for invoice in invoice_array:
    for employee in employee_time_cards:
      if employee.invoice_number==invoice.invoice_number:
        ...
        ##rintemployee_invoices[employee.employee_name], len(employee_invoices[employee.employee_name])
  #    else:
...
#  #rintno_money.keys()
#  #rintno_money.values()
#  #rint"revenue", total_revenue.keys(), total_revenue.values()
#  #rint"callbacks", callbacks.keys(), callbacks.values()
#  #rint"agreements", service_agreements.keys(), service_agreements.values()
